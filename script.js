  ...
  if (storiesSection && !document.getElementById('stories-spacer')){
    ...
    stories.forEach((el, i) => {
      ScrollTrigger.create({
        ...
        onEnter: () => { if (imgEl) imgEl.src = el.dataset.img; },
        onEnterBack: () => { if (imgEl) imgEl.src = el.dataset.img; },
      });
...
(function(){
  ...
  const hoverables = 'a, button, .btn, .pill, .meta-pill, [role="button"]';
  if (cursor){
    document.addEventListener('mousedown', ()=> cursor.classList.add('is-down'));
    document.addEventListener('mouseup', ()=> cursor.classList.remove('is-down'));
    document.addEventListener('mouseover', (e)=>{
      if (e.target && e.target.closest(hoverables)) cursor.classList.add('is-hover');
    });
    document.addEventListener('mouseout', (e)=>{
      if (e.target && e.target.closest(hoverables)) cursor.classList.remove('is-hover');
    });
  }
  ...
  cards.forEach(card=>{
    card.addEventListener('mousemove', (ev)=> setSpot(card, ev));
